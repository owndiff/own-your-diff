...
4. If the picker reports that no TTY is available, run `scripts/present_mcq.py --mcq .owndiff/ownership-mcq.json` and present the MCQs directly in the current chat before waiting for the user.
5. Ask the human to answer in chat with selections such as `q1=c q2=b q3=a`.
6. Run `scripts/submit_answers.py --evaluate ...` with the human's selections.
7. Continue only when `.owndiff/ownership-gate.json` has `agent_may_push_merge_request: true`.
8. Keep `.owndiff/ownership-answer-key.json` local; do not print or paste the answer key into chat.
9. Do not ask the human to open generated files for answering; use the TUI or chat fallback.
10. Do not push, open, or update a merge request while the MCQ gate is `pending_answers` or `failed`.
